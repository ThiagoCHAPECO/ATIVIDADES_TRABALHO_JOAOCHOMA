...
1. Clonar o repositório e acessar a pasta do projeto `Atividade2-TesteFuncional`
...
| Arquivo | Conteúdo |
|---|---|
| [`tests/frete.spec.ts`](tests/frete.spec.ts) | 23 casos (CT-F01 a CT-F23): classes válidas e inválidas de CEP e valor, e os limites de R$ 200,00 (frete grátis) e do valor mínimo |
| [`tests/senha.spec.ts`](tests/senha.spec.ts) | 20 casos (CT-S01 a CT-S20): limites de tamanho (7, 8, 9, 19, 20 e 21 caracteres), classes inválidas de composição e confirmação |
| [`PLANO_DE_TESTES.md`](PLANO_DE_TESTES.md) | Plano com requisitos, classes de equivalência, valores-limite e a tabela de casos de teste |

Resultado de `npm test`: **54 testes aprovados** (43 desta atividade e 11 exemplos fornecidos).
